...
from app.schemas.v1.long_call import LongCallOut, LongCallSortField
from app.services.long_calls import get_long_calls
from fastapi_cache.decorator import cache

router = APIRouter(tags=["long-calls"])


@router.get("/expiry-dates", response_model=ApiResponse[list[str]])
@limiter.limit("30/minute")
@cache(expire=300, key_builder=cache_key_builder, namespace="v1:long_calls:expiry_dates")
async def list_long_calls_expiry_dates(
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    dates = (
        db.query(distinct(LongCall.expiry_date))
        .order_by(LongCall.expiry_date.asc())
        .all()
    )
    return ok(data=[str(row[0]) for row in dates], request=request)


@router.get("", response_model=ApiResponse[List[LongCallOut]])
@limiter.limit("30/minute")
@cache(expire=30, key_builder=cache_key_builder, namespace="v1:long_calls")
async def list_long_calls(
    request: Request,
    ...
    sort_by: LongCallSortField | None = Query(default=None),
