...
JSON_PATHS = [
    DATA_DIR / "best_long_puts_nyse.json",
    DATA_DIR / "best_long_puts_nasdaq.json",
    DATA_DIR / "best_long_puts_arca.json",
]
...
def normalize_row(row: dict) -> dict:
    for field in REQUIRED_FIELDS:
        if field not in row or row[field] is None:
            raise ValueError(f"Missing required field '{field}' in long puts row: {row}")
...
def load_rows_from_file(json_path: Path) -> list[dict]:
    if not json_path.exists():
        raise FileNotFoundError(f"Long puts file not found: {json_path}")
...
def ingest_long_puts(db: Session) -> None:
    logger.info("Replacing long puts from exchange-specific files")

    all_payloads: list[dict] = []

    for json_path in JSON_PATHS:
        logger.info("Loading long puts file", extra={"json_path": str(json_path)})
        rows = load_rows_from_file(json_path)
        for row in rows:
            all_payloads.append(normalize_row(row))

    logger.info("Deleting existing long puts")
    db.query(LongPut).delete()

    logger.info("Inserting fresh long puts", extra={"rows_to_insert": len(all_payloads)})
    db.execute(insert(LongPut), all_payloads)

    db.commit()
    logger.info("Long puts replacement complete", extra={"rows_inserted": len(all_payloads)})
