...
JSON_PATHS = [
    DATA_DIR / "best_spreads_nyse.json",
    DATA_DIR / "best_spreads_nasdaq.json",
    DATA_DIR / "best_spreads_arca.json",
]
...
def normalize_row(row: dict) -> dict:
    ...
    return {
        ...
        "max_profit": normalize_float(row.get("max_profit")),
        "max_profit_per_contract": normalize_float(row.get("max_profit_per_contract")),
        ...
        "option_yield": normalize_float(row.get("option_yield")),
        "roc": normalize_float(row.get("roc")),
        "tot_return": normalize_float(row.get("tot_return")),
...
def ingest_spread_options(db: Session):
    ...
    logger.info("Replacing spread options from exchange-specific files")

    all_payloads: list[dict] = []

    for json_path in JSON_PATHS:
        logger.info("Loading spread options file", extra={"json_path": str(json_path)})
        rows = load_rows_from_file(json_path)

        for row in rows:
            all_payloads.append(normalize_row(row))

    logger.info("Deleting existing spread options")
    db.query(SpreadOption).delete()

    logger.info("Inserting fresh spread options", extra={"rows_to_insert": len(all_payloads)})
    db.execute(insert(SpreadOption), all_payloads)

    db.commit()
    logger.info("Spread options replacement complete", extra={"rows_inserted": len(all_payloads)})
